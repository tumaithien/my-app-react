...
const HeaderMain = () => {
  ...
  const favoriteCount =
    photos.length > 0
      ...
      : 0;
  const cartItemCount = cartItem.length > 0 ? cartItem.length : 0;
  return (
    <div className="p-4 bg-white shadow-md flex items-center justify-center">
      ...
      <div className="ml-auto mr-5 flex items-center gap-5">
        <span className="relative w-8">
          ...
          <span className="w-5 h-5 rounded-full flex items-center justify-center text-white bg-pink-400 text-xs absolute right-0 top-0 translate-x-2/4">
            {cartItemCount || 0}
          </span>
        </span>
        <span className="relative w-8">
